...
import fr.ensicaen.service.impl.AccountService;
...
public class P2PBean extends AbstractBean {
    ...
    public String getRecipientText() {
        if (this.recipientAccount == null) {
            ...
            return String.format(
                    "%s - %s - %d",
                    this.recipientAccount.getClient().getName(),
                    this.recipientAccount.getAccountType().getName(),
                    this.recipientAccount.getIdAccount()
            );
        }
    }
    ...
    /**
     * Fausse lecture de tag, on prend un compte aléatoire dans la base de données et on fait une transaction
     */
    public void readTag() {
